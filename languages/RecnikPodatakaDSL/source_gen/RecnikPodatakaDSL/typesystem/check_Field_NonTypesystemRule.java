...
public class check_Field_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode field, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    SNode parentNode = (SNode) SNodeOperations.getParent(field);
    for (SNode fil : ListSequence.fromList(SLinkOperations.getChildren(parentNode, LINKS.fields$3GOB))) {
      if (field != fil) {
        if (SPropertyOperations.getString(fil, PROPS.name$MnvL).equalsIgnoreCase(SPropertyOperations.getString(field, PROPS.name$MnvL))) {
          {
            final MessageTarget errorTarget = new NodeMessageTarget();
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(field, "Field with this name already exists in this structure", "r:24ac4287-029c-4578-b1eb-0b9b6159757c(RecnikPodatakaDSL.typesystem)", "865134009559750694", null, errorTarget);
          }
        }
      }
    }
